...
public class BoardControllerTest {
	...
	// 게시물 목록 테스트
		...
		public void testListBoard() throws Exception{
		    ...
		    // 여기서는 service.listBoard() 호출이 boardList 객체를 반환하도록 설정합니다.
		    ...
		    // 여기서는 /board/list 엔드포인트에 GET 요청을 수행합니다.
		    this.mockMvc.perform(get("/board/list"))    
		    
		    // Then: When에서 수행한 행동의 결과를 검증합니다.
		    // 기대한 결과가 실제로 발생했는지 확인합니다.
		    // 예: 반환된 값이 예상한 값인지, 상태 코드가 올바른지, 특정 뷰가 렌더링되었는지 확인합니다.
		    .andExpect(status().isOk())  // 응답 상태가 200인지 확인합니다.
		    .andExpect(view().name("board/boardList"))  // board/boardList.jsp로 이동하는지 확인합니다.
		    .andExpect(model().attributeExists("boardList"))  // model에 boardList가 있는지 확인합니다.
		    .andExpect(model().attribute("boardList", boardList));  // model의 boardList가 우리가 설정한 boardList와 일치하는지 확인합니다.
		}
		...
		public void testCreateBoard() throws Exception {
		    ...
		    // BoardVo 객체를 생성하고, service.createBoard 메서드가 호출될 때 1을 반환하도록 설정.
			// 이는 데이터베이스에 새 게시물을 삽입한 후 성공적으로 생성된 게시물의 ID를 반환하는 것을 모킹(모방)
		    BoardVo boardVo = new BoardVo(0, "컨트롤러에서 등록", "내용", "java");
		    Mockito.when(service.createBoard(Mockito.any(BoardVo.class))).thenReturn(1);

		    // When: /createBoard라는 요청을 보내는 행동을 함.
		    // 요청 시, BoardVo 객체의 title, content, memberId 파라미터를 함께 전달한다.
		    ...
		    // 그리고 리다이렉트되는 URL이 "/boardList"인지 확인한다.
		    .andExpect(status().is3xxRedirection())
		    .andExpect(redirectedUrl("/board/list")); // 게시물 목록요청
		}
		...
		public void testUpdateBoard() throws Exception{
		    // Given: 테스트를 위한 전제 조건을 설정합니다.
		    // 여기서는 service.updateBoard() 호출이 1을 반환하도록 설정합니다.
		    ...
		    MemberVo memberVo = new MemberVo("java",  "name", "email");
		    MockHttpSession session = new MockHttpSession();
		    session.setAttribute("memberVo", memberVo);

		    // When: 실제 테스트하는 동작을 수행합니다.
		    // 여기서는 /updateBoard 엔드포인트에 POST 요청을 수행합니다.
		    // 요청 시, BoardVo 객체의 bno, title, content, memberId 파라미터를 전달하고, 세션 정보를 포함합니다.	 
			...
			// Then: When에서 수행한 행동의 결과를 검증합니다.
		    // 기대한 결과가 실제로 발생했는지 확인합니다.
		    // 예: 상태 코드가 3xx 리다이렉션인지, 리다이렉트되는 URL이 "/boardList"인지 확인합니다.
			.andExpect(status().is3xxRedirection())		
			.andExpect(redirectedUrl("/board/list"));	// 게시물 목록 요청
		}
